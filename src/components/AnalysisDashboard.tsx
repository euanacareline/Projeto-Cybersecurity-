...
export const AnalysisDashboard = ({ analysis }: AnalysisDashboardProps) => {
  ...
  return (
    ...
    >
      {/* Header Stat Bar */}
      <div className={cn(
        "grid grid-cols-1 gap-px bg-zinc-800 border border-zinc-800 rounded-lg overflow-hidden shrink-0",
...
const StatItem = ({ label, value, variant }: { label: string, value: string, variant?: any }) => (
  <div className="bg-[#1a1a1a] p-4 flex flex-col gap-1">
    <span className="text-[9px] font-mono uppercase tracking-widest text-zinc-500 opacity-60 italic">{label}</span>
    <div className="flex items-center gap-2">
      {variant ? (
        <Badge variant={variant} className="w-full text-center text-xs py-1">{value}</Badge>
      ) : (
